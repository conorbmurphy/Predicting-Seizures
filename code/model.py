...
class Models(object):
    ...
    def remove_zeros(self):
        ...
        OUTPUT: None, updates self.data
            Looks over the first 160 columns of self.data (the channel means)
            and removes all rows with all zeros
        '''
        print 'Dimensions before starting remove_zeros(): {}'\
        ...
        print 'Dimensions after performing remove_zeros(): {}'\
            .format(self.data.shape)
    ...
    def tt_split(self):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = \
            train_test_split(np.array(self.data), self.y, test_size=0.3,
                             random_state=124)
    ...
    def normalize_and_add_dummies(self):
        ...
        self.X_train = ((self.X_train - self.X_train.mean(axis=0)
                         ) / self.X_train.std(axis=0))[:, :-1]
        self.X_test = ((self.X_test - self.X_test.mean(axis=0)
                        ) / self.X_test.std(axis=0))[:, :-1]
        self.test_set = ((self.test_set - self.test_set.mean(axis=0)) /
                         self.test_set.std(axis=0))[:, :-1]

        self.X_train = np.concatenate(
            [self.X_train, pd.get_dummies(X_train_patient)], axis=1)
        self.X_test = np.concatenate(
            [self.X_test, pd.get_dummies(X_test_patient)], axis=1)
        self.test_set = np.concatenate(
            [self.test_set, pd.get_dummies(test_set_patient)], axis=1)
    ...
    def fit(self):
        ...
        self.svm_rbf()
        self.svm_linear()
        # self.svm_grid_search() # Found C:1 and gamma:.01 as best choices at
        # .77 score
        print '-------- Fit for Patient {} Complete --------'\
            .format(self.patient)
    ...
    def logistic_regression(self):
        ...
        print 'Logistic Regression test score for patient {}: {}'\
            .format(self.patient, score)
    ...
    def random_forest(self):
        ...
        prediction = model.predict_proba(self.X_test)[:, 1]
        score = self._score(self.y_test, prediction)
        print 'Score for Random Forest for patient {}: {}'\
            .format(self.patient, score)
    ...
    def svm_linear(self):
        y_train = (self.y_train * 2) - 1
        y_test = (self.y_test * 2) - 1
        model = svm.SVC(kernel='linear', C=1, gamma=.01,
                        probability=True, class_weight='balanced')
        scores = cross_val_score(model, self.X_train, self.y_train, cv=5,
                                 scoring='roc_auc', n_jobs=-1)
    ...
    def svm_grid_search(self):
        ...
        print 'Best model found has a score of {} with the parameters {}'\
            .format(model_gs.best_score_, model_gs.best_params_)
    ...
    def plot_ROC(self, models):
        ...
        plt.title(
            'Receiver operating characteristic for patient {}'
            .format(self.patient))
    ...
    def get_feature_importances(self, dest_file=None):
        ...
        fi_wav = ('Wavelet Transforms', fi[160:560].sum())
        # 118 MOM calcuations
        fi_mom = ('Method of Moments', fi[560:678].sum())
        fi_ent = ('Entropy Calculations', fi[678:694].sum())  # 16 entropy
        fi_cor = ('Pearson Correleation', fi[
                  694:816].sum())  # 122 correlations
        # 3 patient number dummies
        fi_pat = ('Patient Numbers', fi[816:].sum())
...
def morlet(n_points, a):
    '''
    INPUT:
        n_points: int - Number of points in vector.  Will be centered around 0.
            In scipy.signal.cwt, this will be the nubmer of points that the
            returned vector will have
        a: scalar - width parameter of the wavelet, defining its size
    OUTPUT: vector: Normalized array of length n_points in shape of a morlet
        wavelet

    equation = pi**-0.25 * (exp(1j*w*x) - exp(-0.5*(w**2))) * exp(-0.5*(x**2))
    '''
    vec = np.linspace(-10, 10, n_points)
    wave = (np.pi**-0.25) * (np.exp(-0.5 * vec**2)) * \
        (np.exp(-1j * a * vec) - np.exp(-0.5 * a**2))
...
def create_submission(predict_a, file_name):
    ...
    prediction_df = pd.DataFrame(
        {'File': files, 'Class': prediction}, columns=['File', 'Class'])
...
def import_data(separate=False):
    '''
    INPUT: separate - Boolean, whether to return concatenated or separate data
        frames
    OUTPUT: combined training and test sets
    '''
    a_df = pd.read_csv('data/a_reduced.csv')
    b_df = pd.read_csv('data/b_reduced.csv')
    c_df = pd.read_csv('data/c_reduced.csv')
    ...
    a_test = pd.read_csv('data/a_test_reduced.csv').sort_values(by='817')\
        .drop('817', axis=1)
    b_test = pd.read_csv('data/b_test_reduced.csv').sort_values(by='817')\
        .drop('817', axis=1)
    c_test = pd.read_csv('data/c_test_reduced.csv').sort_values(by='817')\
        .drop('817', axis=1)
...
if __name__ == '__main__':
    ...
    # predictions.columns = ['Logistic_Regression', 'Random_Forest',\
    #    'XGBoost', 'SVM']
